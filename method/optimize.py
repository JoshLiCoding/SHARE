...
def create_smpl_body(pred_rotmat, mean_shape, pred_trans, smooth_traj=False, save=False):
    ...
    if save:
        np.save(f'pred_verts', pred_vert.detach().cpu().numpy())
        np.save(f'pred_faces', pred_faces)
        np.save(f'pred_j3d', pred_j3d.detach().cpu().numpy())
    return pred_j3d, pred_vert
...
def main(folder, frame_1, frame_2):
    ...
    refined_shape = torch.nn.Parameter(mean_shape.clone().detach())
    ...
    optimizer = optim.Adam([refined_trans, refined_shape], lr=lr)
    for it in range(n_iters): 
        ...
        pred_vert = create_smpl_body(pred_rotmat, refined_shape, refined_trans)[1]
        ...
        root_trans = create_smpl_body(pred_rotmat, refined_shape, refined_trans)[0][:, 0, :]
    ...
    save_ply('share_scene.ply', pc_scene.cpu().numpy(), colors_scene)

    create_smpl_body(pred_rotmat, refined_shape, refined_trans, save=True)
