...
def visualize_scene_and_joints(folder):
    ...
    indices = list(range(0, num_meshes, 1))
    start_color = np.array([172, 255, 156])
    end_color = np.array([28, 168, 0])
    for idx, i in enumerate(indices):
        # Gradient from light green [180,255,180] to dark green [0,80,0]
        t = idx / max(len(indices) - 1, 1)
        color = np.round((1 - t) * start_color + t * end_color).astype(int).tolist() + [255]
        server.add_mesh_trimesh(
            mesh=trimesh.Trimesh(
                vertices=share_vert[i],
                faces=faces,
                visual=trimesh.visual.ColorVisuals(
                    vertex_colors=np.tile(color, (share_vert[i].shape[0], 1))
                )
            ),
    ...
    #     server.add_mesh_trimesh(
    #         mesh=trimesh.Trimesh(
    #             vertices=init_share_vert[i],
    #             faces=faces,
    #             visual=trimesh.visual.ColorVisuals(
    #                 vertex_colors=np.tile([255, 169, 77, 255], (init_share_vert[i].shape[0], 1))
    #             )
    #         ),
...
# video: https://www.youtube.com/watch?v=likDxPBn0Ug
