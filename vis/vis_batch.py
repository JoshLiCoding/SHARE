...
# python vis/vis_batch.py Gym_011_dips3_batch02 --mhmocap --bev --tram --share
# python vis/vis_batch.py LectureHall_021_plankjack1_batch03 --mhmocap --bev --tram --share
def main():
    import argparse
    ...
    if use_mhmocap:
        frames_root = os.path.join('eval/RICH/frames')
        ...
        for i in indices:
            ...
            mesh.visual.vertex_colors = np.tile([0, 255, 255, 255], (mhmocap_verts[i].shape[0], 1))
            server.add_mesh_trimesh(
                mesh=mesh,
    ...
    if use_bev:
        ...
        for i in indices:
            ...
            mesh.visual.vertex_colors = np.tile([255, 0, 255, 255], (bev_verts[i].shape[0], 1))
            server.add_mesh_trimesh(
                mesh=mesh,
                name=f"bev_mesh_{i}",
            )
        ...
        #     )
    if use_tram:
        frames_root = os.path.join('eval/RICH/tram_results')
        frames_dir = os.path.join(frames_root, f"{base}_cam{cam_num}_batch{batch}")
        ...
        for i in indices:
            ...
            mesh.visual.vertex_colors = np.tile([0, 255, 0, 255], (tram_verts[i].shape[0], 1))
            server.add_mesh_trimesh(
                mesh=mesh,
                name=f"tram_mesh_{i}",
            )
        ...
        #     )
    if use_share:
        frame_root = os.path.join('eval/RICH/tram_results')
        ...
        for i in indices:
            ...
            mesh.visual.vertex_colors = np.tile([255, 255, 0, 255], (share_verts[i].shape[0], 1))
            server.add_mesh_trimesh(
                mesh=mesh,
